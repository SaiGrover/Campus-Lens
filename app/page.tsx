...
function Landing({ onEnter, onReport, onNavigate }: { onEnter: () => void; onReport: () => void; onNavigate: (view: View) => void }) {
  ...
  return <div className="landing-page">
    <nav className="landing-nav" aria-label="Landing page navigation">
      ...
      <div className="landing-links">{links.map((link, index) => <motion.button custom={index + 1} initial="hidden" animate="visible" variants={fadeDown} key={link.view} onClick={() => navigate(link.view)}>{link.label}</motion.button>)}</div>
