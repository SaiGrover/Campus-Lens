...
function features(value: string) {
  ...
  const terms = [...tokens, ...tokens.slice(0, -1).map((token, index) => `${token} ${tokens[index + 1]}`)];
  const counts = new Map<number, number>();
  for (const term of terms) {
    const index = model.vocabulary[term];
    if (index !== undefined) counts.set(index, (counts.get(index) ?? 0) + 1);
  }
  const weighted = [...counts.entries()].map(([index, count]) => [index, (1 + Math.log(count)) * model.idf[index]] as const);
...
export function classifyComplaint(value: string): { category: Category; confidence: number; algorithm: string } {
  const vector = features(value);
  const scores = model.classes.map((_, classIndex) => {
    let score = model.classLogPrior[classIndex];
    for (const [featureIndex, weight] of vector) score += weight * model.featureLogProb[classIndex][featureIndex];
    return score;
  });
...
export function redactPii(value: string) {
  return value
    ...
    .replace(/\b(?:roll|enrol(?:lment)?)\s*(?:no\.?|number)?\s*[:#-]?\s*[A-Z0-9/-]{5,}\b/gi, "[student id removed]");
}
...
export function findDuplicates(candidate: Pick<Complaint, "text" | "category" | "facility" | "location">, complaints: Complaint[]) {
  return complaints
    .map((complaint) => {
      const sameCategory = complaint.category === candidate.category ? 0.24 : 0;
      const samePlace = normalizeText(complaint.facility ?? complaint.location) === normalizeText(candidate.facility ?? candidate.location) ? 0.26 : 0;
...
export function predictRisk(rating: number, duplicates: number) {
  return duplicates >= 4 || rating === 1 ? "CRITICAL" : duplicates >= 2 || rating === 2 ? "HIGH" : rating === 3 ? "MEDIUM" : "LOW";
}

export function estimateResolution(rating: number, duplicates: number, occupancy = 55) {
  return Number(Math.max(0.4, 1 + (6 - rating) * 0.72 + Math.min(duplicates, 5) * 0.35 + occupancy * 0.018).toFixed(1));
}
...
export function validateComplaintInput(input: unknown) {
  ...
  if (title.length < 4 || title.length > 100) return { ok: false as const, error: "Title must contain 4–100 characters." };
  if (text.length < 12 || text.length > 1200) return { ok: false as const, error: "Complaint must contain 12–1200 characters." };
  if (!facility) return { ok: false as const, error: "A facility is required." };
  if (!category) return { ok: false as const, error: "Select a valid category." };
  if (!Number.isInteger(rating) || rating < 1 || rating > 5) return { ok: false as const, error: "Impact rating must be between 1 and 5." };
  return { ok: true as const, value: { ...body, title, text: redactPii(text), facility, category, rating } };
}
